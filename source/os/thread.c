...
#define MAX_STATCK_SIZE				(768 * 1024)
...
void bkprocess(int signum)
{
    ...
    logPrintf("System error, Stack trace:\n");
    for(i=0;i<size;++i) logPrintf("%d %s \n",i,funs[i]);
    free(funs);
    //exit(1);
}
...
void osBacktrace()
{
	signal(SIGSEGV,bkprocess);
	signal(SIGABRT,bkprocess);
}
